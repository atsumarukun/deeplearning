...
class FashionMnistNet(BaseNet):
    def __init__(self, input_dim=(1, 28, 28), conv_params={"filter_num": 32, "filter_size": 5, "stride": 1, "pad": 0}, hidden_size=100, output_size=10, weight_init_std=0.01):
        super().__init__()
        ...
        self.layers["Affine1"] = Affine(self.params["W2"], self.params["b2"])
        self.layers["ReLU2"] = ReLU()
        # self.layers["Dropout2"] = Dropout(0.3)
        self.layers["Affine2"] = Affine(self.params["W3"], self.params["b3"])
        self.lastLayer = SoftmaxWithLoss()
